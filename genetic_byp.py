...
def mutateGene(gene):
    ...
    for i in range(0, len(choiceInd)):
        if choiceInd[i] == 1:
            ...
            mutated_gene[10] = random.choice([0,1])
...
for i in range(0, popSize):
    ...
    n_conv = random.randrange(0, 4)
    fcn_layer = random.choice([1, 2, 3])
    dropout = random.uniform(0, 0.5)
    epoch = random.choice([5, 10, 15, 20])
    byp = random.choice([0,1])
    nextGeneration.append([fitness, lr, init_W, opt, actF, kernel_size, [conv_layer, n_conv], fcn_layer, dropout, epoch, byp, i])
